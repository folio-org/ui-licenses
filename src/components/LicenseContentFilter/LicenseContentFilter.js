...
import { licesnseContentOptions } from '../../constants';

const LicenseContentFieldArray = ({ handleSubmit }) => {
  const intl = useIntl();
  const translatedContentOptions = licesnseContentOptions.map((e) => {
  ...
  return (
    <FieldArray name="licenseContent">
      {({ fields }) => (
        <>
          {fields?.map((filter, index) => {
            return (
              <div key={index}>
                {values?.licenseContent[index]?.grouping === '&&' && (
                  <Label>
...
const LicenseContentFilter = ({
  ...
  const updateFilters = (values) => {
    const kiwtQueryFilterShape = values?.licenseContent?.reduce((acc, curr) => {
      let newAcc = [];
    ...
    if (values?.licenseContent?.every(validateFilter)) {
  ...
  return (
    <Form
      initialValues={{
        licenseContent: licenseContentFilters.length
          ? parseQueryString(licenseContentFilters)
          : [{}],
      }}
    ...
    >
      {({ handleSubmit }) => (
        <form id="license-content-form" onSubmit={handleSubmit}>
          <LicenseContentFieldArray
...
LicenseContentFieldArray.propTypes = {
  handleSubmit: PropTypes.func,
};

LicenseContentFilter.propTypes = {
  licenseContentFilters: PropTypes.arrayOf(PropTypes.object),
  filterHandlers: PropTypes.object,
  activeFilters: PropTypes.object,
};

export default LicenseContentFilter;
